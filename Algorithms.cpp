        ...
        for (std::size_t u = 0; u < n; ++u) {
            for (std::size_t v = 0; v < n; ++v) {
                if (adjacencyMatrix[u][v] != 0 && dist[u] != std::numeric_limits<int>::max() &&
                    dist[u] + adjacencyMatrix[u][v] < dist[v]) {
                    std::vector<bool> visited(n, false);
                    std::size_t cycleNode = static_cast<std::size_t>(v);

                    for (int k = 0; k < n; ++k) {
                        cycleNode = static_cast<std::size_t>(predecessor[cycleNode]);
                    }

                    std::vector<std::size_t> cyclePath;
                    for (std::size_t current = cycleNode;; current = static_cast<std::size_t>(predecessor[current])) {
                        cyclePath.push_back(current);
                        if (current == cycleNode && cyclePath.size() > 1) {
                            break;
                        }
                    }
                    ...
                    std::ostringstream oss;
                    oss << "The negative cycle is: ";
                    for (std::size_t k = 0; k < cyclePath.size(); ++k) {
                        if (k != 0) {
                            oss << "->";
                        }
                        oss << cyclePath[k];
